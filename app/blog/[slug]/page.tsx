  ...
  return (
    <div className="container mx-auto px-6 pt-32 pb-20">
      <div className="max-w-4xl mx-auto">
        <Link
          href="/blog"
          className="inline-flex items-center gap-3 text-text-muted hover:text-white transition-all mb-12 group font-mono text-[10px] uppercase tracking-widest"
        >
          <ArrowLeft
            size={16}
        ...
        <header className="mb-16">
          <div className="flex items-center gap-6 text-[10px] font-mono uppercase tracking-[0.2em] text-[#64748B] mb-8">
            <span className="text-white border border-white/10 px-3 py-1 bg-white/5">
              {post.category}
            </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 border-t border-white/10 pt-12">
          <div className="lg:col-span-3">
            <div className="sticky top-32 space-y-12">
              <div>
                <h4 className="text-[10px] font-mono uppercase tracking-[0.3em] text-[#64748B] mb-4">
                  Author
                </h4>
          ...
          <div className="lg:col-span-9">
                ...
                if (block.type === "header") {
                  return (
                    <div key={i} className="pt-6">
                      <h2 className="text-2xl md:text-3xl font-sans font-bold text-[#F8F9FA] tracking-tight mb-6">
                        {block.text}
                      </h2>
                    ...
                    <ul key={i} className="space-y-6">
                      {block.items.map((item: string, j: number) => (
                        <li key={j} className="flex gap-4 group">
                          <span className="text-white/20 font-mono text-xs pt-1">
                            0{j + 1}
                          </span>
                          <span className="text-text-secondary text-lg leading-relaxed group-hover:text-white transition-colors">
                            {item}
                          </span>
                        </li>
                      ))}
            ...
            <footer className="mt-20 pt-10 border-t border-white/10">
              <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                ...
                <Link
                  href="/contact"
                  className="px-10 py-3.5 bg-white text-black rounded-full font-bold text-xs uppercase tracking-widest hover:scale-105 transition-all"
                >
                  Discuss this Log
                </Link>
              </div>
